...
**Задачи:**
- [ ] `go mod init`
- [ ] Структура директорий согласно разделу "Структура проекта"
...
- [ ] internal/pkg/postgres — подключение к БД
...
**Задачи:**
- [ ] internal/domain — все доменные структуры
- [ ] migrations/ — начальная миграция со всеми таблицами
...
### Этап 3: Модуль Catalog (Services & Groups)
**Цель:** CRUD для сервисов и групп сервисов
...
**Результат:** можно создавать/читать/обновлять/удалять сервисы через API
...
**Задачи:**
- [ ] internal/identity — JWT аутентификация
...
### Этап 5: Модуль Incidents
**Цель:** управление инцидентами

**Задачи:**
- [ ] internal/incidents — полная реализация
- [ ] Связь инцидентов с сервисами (many-to-many)
- [ ] Timeline updates для инцидента
- [ ] Публичный эндпоинт статуса (GET /api/v1/status)
- [ ] Тесты

**Результат:** полный цикл работы с инцидентами

---

### Этап 6: Модуль Notifications
**Цель:** уведомления о инцидентах

**Задачи:**
- [ ] internal/notifications — подписки, dispatcher
- [ ] Email sender (SMTP)
- [ ] Telegram sender
- [ ] Фоновая отправка (горутины + каналы или простая очередь)
- [ ] Тесты

**Результат:** пользователи получают уведомления об инцидентах
