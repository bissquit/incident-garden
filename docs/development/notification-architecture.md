...
### 2.8. Email канал создаётся автоматически при регистрации

**Решение:** При регистрации пользователя автоматически создаётся email канал с `is_verified = true`. Этот канал использует email из регистрации.

**Обоснование:**

| Вопрос | Ответ |
|--------|-------|
| Зачем пользователю создавать канал вручную? | Незачем — email уже известен |
| Зачем верифицировать registration email? | Незачем — он уже подтверждён фактом регистрации |
| Что если у пользователя нет email? | Невозможно — email обязателен для регистрации |

**Поведение:**
- Пользователь регистрируется → автоматически получает email канал (verified)
- Может сразу настраивать подписки
- Может отключить канал, если не нужны email-уведомления
- Дополнительные email адреса — создаются вручную, требуют верификации кодом

**Что происходит при смене email пользователя:**
- Текущее решение: канал остаётся со старым email
- Пользователь может удалить старый канал и создать новый
- (Опционально в будущем: hook `OnUserEmailChanged` для автоматической синхронизации)

**Реализация:**
- При регистрации identity module вызывает hook `OnUserCreated`
- Notifications module создаёт email канал
- Миграция создаёт каналы для существующих пользователей

**Keycloak/OIDC (будущее):**
- Email приходит из claims токена
- Если IDP говорит `email_verified=true`, доверяем
- Автоматически создаём verified канал
- Та же логика, что и для локальной регистрации
...
- From address в ENV
- Batch size (по умолчанию 50, настраивается)

**Два типа email каналов:**

| Тип | Создание | Верификация | Описание |
|-----|----------|-------------|----------|
| Registration email | Автоматически | Не требуется | Создаётся при регистрации, `is_verified=true` |
| Дополнительный email | Вручную | 6-значный код | Любой другой email адрес |

**Пользовательский flow (registration email):**
1. Пользователь регистрируется в системе
2. Email канал создаётся автоматически (verified)
3. Пользователь настраивает подписки

**Пользовательский flow (дополнительный email):**
1. Ввод email в настройках → POST /me/channels
2. Система отправляет 6-значный код на указанный адрес
3. Ввод кода → POST /me/channels/{id}/verify → канал верифицирован

**Защита от дубликатов:**
- Нельзя создать два канала с одинаковым email для одного пользователя
- При попытке → 409 Conflict

**Отправка:**
- Один email на событие, получатели в BCC
- Батчи по N (настраивается через `NOTIFICATIONS_EMAIL_BATCH_SIZE`, default 50)
- При M получателях = ceil(M/N) писем
...
-- Уникальный индекс для предотвращения дубликатов email каналов
-- (один пользователь не может иметь два канала с одинаковым email)
CREATE UNIQUE INDEX idx_notification_channels_user_email_target
    ON notification_channels(user_id, type, target)
    WHERE type = 'email';
```

### Миграция: создание email каналов для существующих пользователей

```sql
-- При первой установке или апгрейде: создать email каналы для всех пользователей
...
SELECT
    ...
    u.email,
    true,           -- is_enabled
    true,           -- is_verified (registration email = trusted)
    false,          -- subscribe_to_all_services
    u.created_at,   -- use user's registration time
    NOW()
FROM users u
WHERE NOT EXISTS (
    SELECT 1
...
**Первое посещение (email канал уже есть):**

У нового пользователя уже есть email канал (создан при регистрации), но нет подписок.
...
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  ℹ️  Вы пока не подписаны ни на один сервис              │    │
│  │                                                         │    │
│  │  Выберите сервисы ниже, чтобы получать уведомления      │    │
│  │  о событиях (инцидентах, плановых работах).             │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│                           ┌───────┐                             │
│                           │ Email │                             │
...
**Важно для UI:**
- Email канал нельзя удалить через UI (можно только отключить)
- Или: можно удалить, но появится предупреждение "Вы потеряете возможность получать email уведомления"
...
**Email верификация (только для дополнительных адресов):**

Эта модалка показывается только при добавлении дополнительного email адреса, отличного от registration email.
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Мы отправили код на alerts@example.com                         │
│                                                                 │
...
**Когда показывается:**
- Пользователь добавляет email канал с адресом, отличным от registration email

**Когда НЕ показывается:**
- Registration email канал (создаётся автоматически, уже verified)
- Telegram/Mattermost (верифицируются тестовым сообщением, не кодом)
...
|------------|---------------------|-----------|
| Автосинхронизация email канала при смене email пользователя | Смена email не реализована, решается вручную | Medium |
...
| Кастомизация шаблонов уведомлений через UI | Шаблоны встроены в код, меняются редко | Low |

### Подготовка к будущим изменениям

**Автосинхронизация email при смене email пользователя:**
- Hook `OnUserEmailChanged(ctx, userID, oldEmail, newEmail)`
- Найти registration email канал (target = oldEmail)
- Обновить target на newEmail
- Или: удалить старый, создать новый

**Keycloak/OIDC интеграция:**
- При первом входе через OIDC — создать email канал с email из claims
- Если `email_verified=true` в claims → `is_verified=true`
- Та же логика через hook `OnUserCreated`
...
- Таблица `notification_templates(channel_type, message_type, subject_template, body_template)`
- Admin API для CRUD шаблонов
- Renderer загружает из БД с fallback на встроенные шаблоны
...
|--------|------|-----------|
| 1.1 | 2024-02 | Добавлено: автоматическое создание email канала при регистрации (секция 2.8), обновлён flow для Email (секция 3), обновлён UI для первого посещения (секция 12) |
